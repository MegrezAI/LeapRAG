...
export function PaginationWithLinks({
  ...
  const pageSizeOptions = pageSizeSelectOptions?.pageSizeOptions || [10, 20, 30, 50];
...
function SelectRowsPerPage({
  ...
  return (
    <div className="flex items-center gap-4">
      <span className="whitespace-nowrap text-sm">{t('Rows per page')}</span>

      <Select value={String(pageSize)} onValueChange={(value) => setPageSize(Number(value))}>
        <SelectTrigger>
          <SelectValue placeholder={t('Select page size')}>{String(pageSize)}</SelectValue>
        </SelectTrigger>
        <SelectContent>
          {options.map((option) => (
