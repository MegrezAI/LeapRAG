...
class PushNotificationListener():
    ...
    def start(self):
        try:
            ...
            logging.info("======= push notification listener started =======")
    ...
    async def handle_validation_check(self, request: Request):
        ...
        logging.info(f"\npush notification verification received => {validation_token}")
    ...
    async def handle_notification(self, request: Request):
        ...
        try:
            if not await self.notification_receiver_auth.verify_push_notification(request):
                logging.error("push notification verification failed")
                return
        except Exception as e:
            logging.error(f"error verifying push notification: {e}")
            return
