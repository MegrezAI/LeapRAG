...
class PushNotificationSenderAuth(PushNotificationAuth):
    ...
    async def verify_push_notification_url(url: str) -> bool:
        async with httpx.AsyncClient(timeout=10) as client:
            try:
                ...
                logging.info(f"Verified push-notification URL: {url} => {is_verified}")
                return is_verified
            except Exception as e:
                logging.info(f"Error during sending push-notification for URL {url}: {e}")
    ...
    def generate_jwk(self):
        ...
        self.private_key_pem = key.export_to_pem(private_key=True, password=None).decode('utf-8')
    ...
    async def send_push_notification(self, url: str, data: dict[str, Any]):
        ...
        async with httpx.AsyncClient(timeout=10) as client:
            try:
                ...
                response.raise_for_status()
                logging.info(f"Push-notification sent for URL: {url}")
            except Exception as e:
                logging.info(f"Error during sending push-notification for URL {url}: {e}")
