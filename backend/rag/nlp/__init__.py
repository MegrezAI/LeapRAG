...
def hierarchical_merge(bull, sections, chunk_token_num):
    if not sections or bull < 0:
        return []
    if isinstance(sections[0], type("")):
    ...
    logging.info(f"hierarchical_merge sections count={len(sections)}")

    res = [[]]
    num = [0]
    tag_pattern = r"@@[0-9]+.*"
    current_titles = [None] * bullets_size  # 存储每个层级的当前标题
    current_content = []

    def add_to_block(text, level=-1):
        tokens = num_tokens_from_string(re.sub(tag_pattern, "", text))
        if num[-1] + tokens > chunk_token_num and level == -1:  # 只有非标题内容才检查token限制
            # 如果当前块已满，创建新块
            res.append([])
            num.append(0)
            # 复制所有有效的标题到新块
            for title in current_titles:
                if title:
                    res[-1].append(title)
                    num[-1] += num_tokens_from_string(re.sub(tag_pattern, "", title))
        res[-1].append(text)
        num[-1] += tokens

    # 处理文案（第一个块）
    first_block_end = next((i for i, text in enumerate(sections)
                            if any(re.match(p, text.strip()) for p in BULLET_PATTERN[bull])), 0)

    if first_block_end > 0:
        for text in sections[:first_block_end]:
            add_to_block(text)
        res.append([])
        num.append(0)

    # 处理正文内容
    for i, text in enumerate(sections[first_block_end:], first_block_end):
        # 检查标题层级
        title_level = -1
        for level, patterns in enumerate(BULLET_PATTERN[bull]):
            if any(re.match(p, text.strip()) for p in ([patterns] if isinstance(patterns, str) else patterns)):
                title_level = level
                break

        if title_level >= 0:  # 是标题
            # 如果是更高层级的标题，清除所有低层级的标题
            for j in range(title_level + 1, len(current_titles)):
                current_titles[j] = None

            # 更新当前层级的标题
            current_titles[title_level] = text

            # 如果不是第一个块且当前块不为空，创建新块
            if num[-1] > 0:
                res.append([])
                num.append(0)

            # 添加所有有效的更高层级标题
            for j in range(title_level):
                if current_titles[j]:
                    add_to_block(current_titles[j], j)

            # 添加当前标题
            add_to_block(text, title_level)
            current_content = []
        else:
            # 普通内容，直接添加到当前块
            add_to_block(text)
