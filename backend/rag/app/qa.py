...
class Excel(ExcelParser):
    async def __call__(self, fnm, binary=None, callback=None):
        if not binary:
...
class Docx(DocxParser):
    ...
    async def __call__(self, filename, binary=None, from_page=0, to_page=100000, callback=None):
        self.doc = Document(
...
async def chunk(filename, binary=None, lang="Chinese", callback=None, **kwargs):
    """
    ...
    if re.search(r"\.xlsx?$", filename, re.IGNORECASE):
        ...
        excel_parser = Excel()
        for ii, (q, a) in enumerate(await excel_parser(filename, binary, callback)):
        ...
        while i < len(lines):
            ...
            i += 1
            if len(res) % 999 == 0:
                await callback(len(res) * 0.6 / len(lines), ("Extract Q&A: {}".format(len(res)) + (
                    f"{len(fails)} failure, line: %s..." % (",".join(fails[:3])) if fails else "")))

        if question:
            res.append(beAdoc(deepcopy(doc), question, answer, eng, len(lines)))

        await callback(0.6, ("Extract Q&A: {}".format(len(res)) + (
            f"{len(fails)} failure, line: %s..." % (",".join(fails[:3])) if fails else "")))
        ...
        await callback(0.1, "Start to parse.")
        pdf_parser = Pdf()
        qai_list, tbls = await pdf_parser(filename if not binary else binary,
        ...
        docx_parser = Docx()
        qai_list, tbls = await docx_parser(filename, binary,
                                    from_page=0, to_page=10000, callback=callback)
