...
async def chunk(filename, binary, tenant_id, lang, callback=None, **kwargs):
    ...
    try:
        await callback(0.1, "USE Sequence2Txt LLM to transcription the audio")
        seq2txt_mdl = await LLMBundle.create(tenant_id, LLMType.SPEECH2TEXT, lang=lang)
        ans = seq2txt_mdl.transcription(binary)
        await callback(0.8, "Sequence2Txt LLM respond: %s ..." % ans[:32])
