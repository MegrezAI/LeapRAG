...
async def chunk(filename, binary=None, lang="Chinese", callback=None, **kwargs):
    ...
    if re.search(r"\.xlsx?$", filename, re.IGNORECASE):
        ...
        while i < len(lines):
            ...
            i += 1
            if len(res) % 999 == 0:
                await callback(len(res) * 0.6 / len(lines), ("Extract TAG: {}".format(len(res)) + (
                    f"{len(fails)} failure, line: %s..." % (",".join(fails[:3])) if fails else "")))

        await callback(0.6, ("Extract TAG: {}".format(len(res)) + (
            f"{len(fails)} failure, line: %s..." % (",".join(fails[:3])) if fails else "")))
        ...
        for i, row in enumerate(reader):
            ...
            if len(row) != 2:
                ...
                content = ""
            if len(res) % 999 == 0:
                await callback(len(res) * 0.6 / len(lines), ("Extract Tags: {}".format(len(res)) + (
                    f"{len(fails)} failure, line: %s..." % (",".join(fails[:3])) if fails else "")))
