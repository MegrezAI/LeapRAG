...
logger = logging.getLogger('leaprag.es_conn')
...
class ESConnection(DocStoreConnection):
    def __init__(self):
        self.info = {}
        logger.info(f"Use Elasticsearch {app_config.ES_HOSTS} as the doc engine.")
        for _ in range(ATTEMPT_TIME):
            try:
                self.es = Elasticsearch(
                    app_config.ES_HOSTS.split(","),
                    basic_auth=(app_config.ES_USERNAME, app_config.ES_PASSWORD),
                    verify_certs=False,
                    timeout=600
                )
                if self.es:
                ...
                logger.warning(f"{str(e)}. Waiting Elasticsearch {app_config.ES_HOSTS} to be healthy.")
                time.sleep(5)
        if not self.es.ping():
            msg = f"Elasticsearch {app_config.ES_HOSTS} is unhealthy in 120s."
            logger.error(msg)
            raise Exception(msg)
        ...
        if int(v) < 8:
            ...
            logger.error(msg)
            raise Exception(msg)
        fp_mapping = os.path.join(get_project_base_directory(), "configs", "mapping.json")
        if not os.path.exists(fp_mapping):
            msg = f"Elasticsearch mapping file not found at {fp_mapping}"
            logger.error(msg)
            raise Exception(msg)
        self.mapping = json.load(open(fp_mapping, "r"))
        logger.info(f"Elasticsearch {app_config.ES_HOSTS} is healthy.")
