...
def init_app(app):
    ...
    if app_config.REDIS_USE_SENTINEL:
        assert app_config.REDIS_SENTINELS is not None, "REDIS_SENTINELS must be set when REDIS_USE_SENTINEL is True"
        sentinel_hosts = [
            (node.split(":")[0], int(node.split(":")[1])) for node in app_config.REDIS_SENTINELS.split(",")
        ]
        sentinel = Sentinel(
            sentinel_hosts,
            sentinel_kwargs={
        ...
        master = sentinel.master_for(app_config.REDIS_SENTINEL_SERVICE_NAME, **redis_params)
        redis_client.initialize(master)
    elif app_config.REDIS_USE_CLUSTERS:
        assert app_config.REDIS_CLUSTERS is not None, "REDIS_CLUSTERS must be set when REDIS_USE_CLUSTERS is True"
        nodes = [
        ...
        redis_client.initialize(
            RedisCluster(startup_nodes=nodes, password=app_config.REDIS_CLUSTERS_PASSWORD))  # type: ignore
    else:
        redis_params.update(
            {
