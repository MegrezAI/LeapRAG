...
def get_home_cache_dir():
    dir = os.path.join(os.path.expanduser('~'), ".leaprag")
    try:
        os.mkdir(dir)
    except OSError:
        pass
...
def filename_type(filename):
    ...
    if re.match(
             r".*\.(eml|doc|docx|yml|xml|htm|json|csv|txt|ini|xls|xlsx|wps|rtf|hlp|numbers|key|md|py|js|java|c|cpp|h|php|go|ts|sh|cs|kt|html|sql)$", filename):
        return FileType.DOC.value

    if re.match(
            r".*\.(wav|flac|ape|alac|wavpack|wv|aac|ogg|vorbis|opus|mp3)$", filename):
